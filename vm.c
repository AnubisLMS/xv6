...
void switchuvm(struct proc* p) {
  ...
  cpu->ts.esp0 = (unsigned int) proc->kstack + KSTACKSIZE;
  ltr(SEG_TSS << 3);
  if(p->pgdir == 0)
...
void inituvm(pde_t* pgdir, char* init, unsigned int sz) {
  char* mem;
...
int allocuvm(pde_t* pgdir, unsigned int oldsz, unsigned int newsz) {
  char* mem;
  unsigned int a;
...
int deallocuvm(pde_t* pgdir, unsigned int oldsz, unsigned int newsz) {
  pte_t* pte;
  unsigned int a, pa;
...
int copyout(pde_t* pgdir, unsigned int va, void* p, unsigned int len) {
  char *buf, *pa0;
  unsigned int n, va0;
